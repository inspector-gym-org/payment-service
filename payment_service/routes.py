...
from .models import Payment, PaymentCreate, PaymentUpdate, PaymentWrite
...
async def create_payment(payment_create: PaymentCreate) -> Payment:
    payment = PaymentWrite(**payment_create.dict())
...
async def get_payment(payment_id: str) -> Payment:
    if payment := await payments_collection.find_one({"_id": ObjectId(payment_id)}):
        return payment
...
async def update_payment(payment_id: str, payment_update: PaymentUpdate) -> Payment:
    update_result = await payments_collection.update_one(
        {"_id": ObjectId(payment_id)}, {"$set": jsonable_encoder(payment_update)}
    )

    if update_result.modified_count == 1:
        if payment_json := await payments_collection.find_one(
